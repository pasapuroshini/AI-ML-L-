...
1. 1.  **Start with an Initial Solution**
1.     
1.     * *   This could be a **random solution** or generated using some **heuristic**.
1.     *     
1. 2.  **Generate a Set of Neighboring Solutions**
1.     
1.     * *   Neighborhood is defined based on the problem (e.g., swap cities in TSP).
1.     *     
1. 3.  **Evaluate and Select the Best Neighbor**
1.     
1.     * *   Even if it's **worse** than the current solution, accept it if it's not in the tabu list.
1.     *     
1. 4.  **Update the Tabu List**
1.     
1.     * *   Add the **current solution** to the list to avoid revisiting.
1.     *     
1.     * *   Maintain a **fixed size** for the tabu list (e.g., last 10 moves).
1.     *     
1. 5.  **Repeat the Process**
1.     
1.     * *   Continue for a **predefined number of iterations** or until **convergence** is reached.
1.     *     
...
* *   **Iteration Steps:**
*     
*     1. 1.  Select a random route.
*     1.     
*     1. 2.  Compute the total distance of each neighboring route.
*     1.     
*     1. 3.  Choose the best neighbor **not in tabu list**.
*     1.     
*     1. 4.  Add the current route to the tabu list.
*     1.     
*     1. 5.  Repeat the process.
*     1.     
...
*     * *   **Aspiration Criteria**
*     *     
*     * *   **Intensification/Diversification st**
...
1. 1.  **Step 1:** Start with an initial solution (random or heuristic-based).
1.     
1. 2.  **Step 2:** Evaluate the neighboring solutions.
1.     
1. 3.  **Step 3:** Select the best neighboring solution that is _not_ in the **tabu list**.
1.     
1. 4.  **Step 4:** Add the current solution to the tabu list.
1.     
1. 5.  **Step 5:** Repeat the process until a stopping condition is met (like max iterations or convergence).
1.     
